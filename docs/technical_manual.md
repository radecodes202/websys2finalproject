...
## Project Overview
A Django web application for small-business inventory, sales, and supplier management. It is built in incremental phases and covered by an automated test suite (**80 tests, all passing** on Django 5.2).
...
- `accounts/` – custom `User` model, roles, approval workflow, `RoleRequiredMixin`, dashboard and auth views
...
- **`AuditLog`**: `user` (FK, SET_NULL), `username_snapshot`, `action` (CREATE/UPDATE/DELETE/LOGIN/LOGOUT/LOGIN_FAILED/STATUS_CHANGE/STOCK_ADJUSTMENT/PAYMENT/OTHER), `content_type` (GFK), `object_id`, `object_repr`, `changes` (JSON), `description`, `ip_address`, `user_agent`, `timestamp`, `severity`. Append-only.
...
## Testing
80 tests across `accounts`, `product`, `supplier`, `customer`, `category`, `reports`, and `audit`. Core coverage:
- Auth & dashboard: login, self-registration approval flow, dashboard cards (product count, today's sales, low stock, alerts, profit, pending POs)
...
- Audit trail: model, service, signals, middleware, viewer, CSV export

Commands:
- Full suite: `py manage.py test accounts supplier customer category product reports audit`
- Per app: `py manage.py test <app>.tests` (e.g. `accounts.tests`, `product.tests`, `reports.tests`, `audit.tests`)
...
Schedule a periodic `pg_dump` via Render cron job or a custom management command. Store dumps in durable object storage (e.g., S3, Render Disks).

Example cron command:
```bash
pg_dump -U $DB_USER -h $DB_HOST -p $DB_PORT $DB_NAME > /tmp/backup_$(date +%Y%m%d).sql
```

Restore with:
```bash
psql -U $DB_USER -h $DB_HOST -p $DB_PORT $DB_NAME < backup_20240101.sql
```

### SQLite (Local Development)
Simply back up the `db.sqlite3` file while the server is stopped, or use:
```bash
sqlite3 db.sqlite3 ".backup backup.db"
```

### Django Management Command (Optional)
Create a custom management command `core/management/commands/backup_db.py` that reads `settings.DATABASES` and invokes the appropriate dump utility. Schedule via Render cron or system cron.
